...
# entries = "["
# for itemId in ids_list:
# 	url      = solr + '/select?q=goodreadsId:' + itemId + '&wt=json' #ex. item: 7144
# 	response = json.loads( urlopen(url).read().decode('utf8') )
# 	doc      = response['response']['docs'][0]
# 	authHrefs = doc['author.authors.authorHref']
# 	entry = "('" + str(itemId) +  "', 0, "
# 	for i in range(3):
# 		if i < len(authHrefs):
# 			authId = authHrefs[i].split('/')[-1].split('.')[0].split('-')[0]
# 			entry += "'" + str(authId) + "'"
# 		else:
# 			entry += "'0'"
# 		if i == 2:
